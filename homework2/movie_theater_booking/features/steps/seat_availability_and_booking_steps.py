...
def step_impl(context, movie_title):
    ...
    Seat.objects.create(movie=context.movie,
                        movie_location=1,
...
def step_impl(context):
    ...
    login_successful = context.test.client.login(username="user5678", password="password.5678")
...
def step_impl(context, title, date, time):
    ...
    context.movie = Movie.objects.create(title=title,
                                         description="Black Panther is an available movie.",
                                         release_date="2026-05-20",
                                         duration=181)

    context.test_seat = Seat.objects.create(movie=context.movie,
                                            movie_location=1,
    ...
    context.booked_seat, created = Seat.objects.get_or_create(movie=context.movie,
                                                              movie_location=1,
                                                              movie_time=context.datetime_version,
                                                              seat_row="A",
                                                              seat_col=1,
...
def step_impl(context, seat):
    url = reverse("book_selected_seat", args=(context.movie.id, context.test_seat.id,))
    context.response = context.test.client.post(url, follow=True)

@when("I select seat \"{seat}\" which is already booked")
def step_impl(context, seat):
    url = reverse("book_selected_seat", args=(context.movie.id, context.booked_seat.id,))
    context.response = context.test.client.post(url, follow=True)
